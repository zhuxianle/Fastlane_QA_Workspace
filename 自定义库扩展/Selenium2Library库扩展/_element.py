...
class _ElementKeywords(KeywordGroup):
    ...
    def click_element(self, locator):
        ...
        element = self._apply_style(locator)
        # element = self._element_find(locator, True, True)
        element.click()
...
return !element.dispatchEvent(evt);
    ...
    def _apply_style(self, locator):
        number_of_clicks = 0
        while number_of_clicks < 3:
            try:
                element = self._element_find(locator, True, True)
                self._current_browser().execute_script("arguments[0].style.border='6px solid red'", element)
                return element
            except Exception, e:
                number_of_clicks += 1
                self._warn(e)

    def Send_keys(self, send_str):
        """在当前位置输入字符串"""
        ActionChains(self._current_browser()).send_keys(send_str).perform()
