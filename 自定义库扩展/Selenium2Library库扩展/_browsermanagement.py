...
class _BrowserManagementKeywords(KeywordGroup):
    ...
    def open_browser(self, url, browser='firefox', alias=None,remote_url=False,
        ...
        if remote_url:
            ...
            #增加个逻辑使之本地也可以支持远程执行功能
            if os.path.exists(r"C:\SeleniumGrid_HubUrl.txt"):
                with open(r"C:\SeleniumGrid_HubUrl.txt") as hub_url_f:
                    remote_url = hub_url_f.read()
                    # self._info("RIDE Debug: '%s'" % remote_url)
            if remote_url:
                self._info(u"\t\n通过本地配置文件地址远程执行脚本！\t\n")
                self._info("Opening browser '%s' to base url '%s' through remote server at '%s'"
                    % (browser, url, remote_url))
