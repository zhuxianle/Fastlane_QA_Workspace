...
class Query(object):
    ...
    def Set_SQL_Path(self, filepath):
        """
        设置SQL文件保存的路径。
        必须先设置才能使用后面的关键字。
    ...
    def execute_sql_script(self, sqlScriptFileName):
        ...
        delete 
          from employee_table
        """
        sqlScriptFile = open(self.SQL_filePath + "\\" + sqlScriptFileName)
        cur = None
        try:
            cur = self._dbconnection.cursor()        
            sqlStatement = ''
    ...
    def execute_sql_string(self, sqlString):
        ...
        try:
            cur = self._dbconnection.cursor()
            #self.__execute_sql(cur, sqlString.encode('utf-8'))
    ...
    def decode(self, coustom_str):
        ...
        return coustom_str.encode("utf-8")
