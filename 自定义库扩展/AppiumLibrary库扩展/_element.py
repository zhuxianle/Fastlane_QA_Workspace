...
class _ElementKeywords(KeywordGroup):
    ...
    # new add ---------------------------------------------------------------------
    def get_element_text(self, locator, loglevel='INFO'):
        """
        获取当前元素的文本值。
        ...
        """
        if not self._element_find(locator, True, True):
            self.log_source(loglevel)
            raise AssertionError(u"找不到要获取的元素信息！用例执行失败… …")
            return False
        self._info(u"已找到要获取的元素！")
        return self._element_find(locator, True, True).text
    ...
    def get_window_size(self):
        """
        获取当前屏幕的分辨率（长和宽） （暂时有问题，appium环境下不支持）。
    ...
    def element_should_be_displayed(self, locator, loglevel='INFO'):
        """验证元素必须是可见的,返回True或者False.
        """
        if not self._element_find(locator, True, True).is_displayed():
            self.log_source(loglevel)
            raise AssertionError(u"找不到该元素或者元素不可见！用例执行失败… …")
            return False
        self._info(u"已找到该元素且元素可见！")
        return True

    def element_should_be_selected(self, locator, loglevel='INFO'):
        """返回元素是否被选中,返回True或者False.

        可以用来检查一个复选框或单选按钮被选中。
        """
        if not self._element_find(locator, True, True).is_selected():
            self.log_source(loglevel)
            raise AssertionError(u"找不到该元素或者元素未被选中！用例执行失败… …")
            return False
        self._info(u"已找到该元素且元素已选中！")
        return True
